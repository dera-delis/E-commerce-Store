...
Write-Host "1. Granting Cloud Run Admin role..." -ForegroundColor Green
gcloud projects add-iam-policy-binding $PROJECT_ID `
  --member="serviceAccount:${CLOUD_BUILD_SA}" `
  --role="roles/run.admin"

# Grant Service Account User role (needed to deploy)
Write-Host "2. Granting Service Account User role..." -ForegroundColor Green
gcloud projects add-iam-policy-binding $PROJECT_ID `
  --member="serviceAccount:${CLOUD_BUILD_SA}" `
  --role="roles/iam.serviceAccountUser"

# Grant Cloud Run Invoker role (if needed)
Write-Host "3. Granting Cloud Run Invoker role..." -ForegroundColor Green
gcloud projects add-iam-policy-binding $PROJECT_ID `
  --member="serviceAccount:${CLOUD_BUILD_SA}" `
